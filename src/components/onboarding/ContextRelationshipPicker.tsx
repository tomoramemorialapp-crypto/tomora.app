...
import { contextRelationshipChoices } from '@/lib/contextualAdd';

/** Relationship choices relative to a selected context person (not the anchor). */
export function ContextRelationshipPicker({
  selectedId,
  onSelect,
}: {
  selectedId?: string;
  onSelect: (choice: (typeof contextRelationshipChoices)[number]) => void;
}) {
  return (
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm, justifyContent: 'center' }}>
      {contextRelationshipChoices.map((choice) => {
