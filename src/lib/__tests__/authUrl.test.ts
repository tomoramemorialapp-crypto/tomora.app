...
describe('Supabase auth redirect allow list', () => {
  it('lists production (www + apex) and dev callback URLs for the dashboard', () => {
    const urls = getSupabaseAuthRedirectAllowList();

    expect(urls).toContain('https://www.tomora.app/auth/callback');
    expect(urls).toContain('https://www.tomora.app/auth/callback?next=onboarding');
    expect(urls).toContain('https://www.tomora.app/auth/callback?next=claim');
    expect(urls).toContain('https://www.tomora.app/auth/callback?next=reset-password');
    expect(urls).toContain('https://www.tomora.app/**');
